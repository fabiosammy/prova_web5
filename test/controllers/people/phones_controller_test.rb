require 'test_helper'

class People::PhonesControllerTest < ActionController::TestCase
  setup do
    @person = people(:one)
    @phone = phones(:one)
  end

  test "should get index" do
    get :index, person_id: @person
    assert_response :success
    assert_not_nil assigns(:phones)
  end

  test "should get new" do
    get :new, person_id: @person
    assert_response :success
  end

  test "should create phone" do
    assert_difference('Phone.count') do
      post :create, person_id: @person, phone: {number: "8823-5678", person_id: @person}
    end
    assert_redirected_to person_phones_path(assigns(:person))
  end

  test "should show phone" do
    get :show, person_id: @person, id: @phone
    assert_response :success
  end

  test "should get edit" do
    get :edit, person_id: @person, id: @phone
    assert_response :success
  end

  test "should update phone" do
    patch :update, person_id: @person, id: @phone, phone: { number:'99000099' }
    assert_redirected_to person_phones_path(@person, assigns(:phones))
    assert_equal '99000099', Phone.find(@phone.id).number
  end

  test "should destroy phone" do
    assert_difference('Phone.count', -1) do
      delete :destroy, person_id: @person, id: @phone
    end  
  end

end
